...
if (!(CoreFoundation && Security && Foundation)) {
  throw new Error("CoreFoundation, Security, or Foundation not found");
}

const CFStringGetCStringPtrImpl = CoreFoundation.findExportByName(
  "CFStringGetCStringPtr",
);
const CFStringGetLengthImpl =
  CoreFoundation.findExportByName("CFStringGetLength");
...
const CFStringGetCStringPtr = new NativeFunction(
  CFStringGetCStringPtrImpl,
  "pointer",
  ["pointer", "int"],
);
const CFStringGetLength = new NativeFunction(CFStringGetLengthImpl, "int", [
  "pointer",
]);
...
const SecItemAdd = new NativeFunction(
  Security.findExportByName("SecItemAdd")!,
  "int",
  ["pointer", "pointer"],
);
const SecItemUpdate = new NativeFunction(
  Security.findExportByName("SecItemUpdate")!,
  "int",
  ["pointer", "pointer"],
);
const SecItemDelete = new NativeFunction(
  Security.findExportByName("SecItemDelete")!,
  "int",
  ["pointer"],
);
const SecAccessControlCreateWithFlags = new NativeFunction(
  Security.findExportByName("SecAccessControlCreateWithFlags")!,
  "pointer",
    ...
    for (let i = 0, size = arr.count(); i < size; i++) {
      ...
      const readable: { [key: string]: any } = {
        ...
        accessibleAttribute: constantLookup(
          item.objectForKey_(C.kSecAttrAccessible),
        ),
      };
      ...
      readable["account"] = readableAccount(
        item.objectForKey_(C.kSecAttrAccount),
      );
