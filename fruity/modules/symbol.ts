...
function swiftDemangle(name: string) {
  if (typeof cachedSwiftDemangler === "function")
    return cachedSwiftDemangler(name);

  const mod = loadDemangler();
  if (!mod) {
    console.warn("Failed to load swift demangler, system might be too old");
    return null;
  }

  const p = mod.findExportByName("swift_demangle_getDemangledName");
  if (!p) {
    console.warn("Failed to find swift demangler function");
    cachedSwiftDemangler = () => null;
    return null;
  }

  const demangle = new NativeFunction(p, "uint", [
    "pointer",
    "pointer",
    "uint",
  ]);
